...
const game = {
  ...
  scored: ['Rohit', 'Warner', 'Finch', 'Rohit'],
...
const printGoals = function (...players) {
  console.log(...players);
  console.log(`Total Goal Scored : ${players.length}`);
};
...
//1
const uniqueGameEventsSets = new Set([...gameEvents.values()]);
const uniqueGameEvents = [...uniqueGameEventsSets];
...
console.log(
  `An event happened, on every ${time.pop() / gameEvents.size} minutes`
);
...
const toCameCase = function (text) {
  const arr = text.split('\n');
  for (const [i, str] of arr.entries()) {
    const [firstWord, secondWord] = str.toLowerCase().trim().split('_');
    let newsecondWord = secondWord.replace(
      secondWord[0],
      secondWord[0].toUpperCase()
    );
    const finalStr = firstWord + newsecondWord;
